...
\label{lemma:awkward_factorization}
\hypertarget{lemma:awkward_factorization}{}
\begin{tcolorbox}
\textbf{Lemma}

For any $n \in \mathbb{N}^+$, for any awkward number series $S_{1,n}$, for any positive integer $x$, it is the case that $x = c \displaystyle \prod_{i = 0}^{j} s_i ^ {p_i}$ where $1 \leq c < s_0$, $s_j \leq x < s_{j + 1}$ and $p_i \in \mathbb{N}$ for all $i$.\\

\noindent We call $c \displaystyle \prod_{i = 0}^{j} s_i ^ {p_i}$ the \textit{awkward factorization} of $x$ in $S_{1, n}$.
\end{tcolorbox}
