...
\\
\textit{Base Case} ($length = 1$)\\
Let $l = 1$ be the length of the walk, giving us $l = i - j = 1$ and $i = j + 1$.\\
Substituting gives us:
\begin{center}
$\omega(p_j, l) = \omega(p_j, i - j) = \omega(p_j, 1) = p_{j+1} = p_i$
\end{center} 
\noindent
\\
\textit{Hypothesis}\\
...
\noindent
\textit{Proof}\\
Our lemma proved that $\omega(p_j, i - j) = p_i$, for $0 \leq j < i < n$.
