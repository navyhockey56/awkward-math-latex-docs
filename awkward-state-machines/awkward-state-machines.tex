...
Before we state our next lemma, let us get some notation out of the way. For any integers $k, j$, and non-zero integer, $n$, we write $j$ $mod$ $n = k$ (read "$j$ modulo $n$ is $k$") to imply that $j = mn + k$ for some integer $m$.\\
\\
\textbf{Lemma}\\
