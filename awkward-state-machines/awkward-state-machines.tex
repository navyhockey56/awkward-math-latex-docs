...
\noindent
\\
The first part of our definition is almost exactly the same as our definition of cycle graphs. However, instead of talking about points in a graph, and the edges between those points; we define the states of a machine, and the transitions between those states. With such similar definitions, it seems reasonable that we can represent the states of an activation cycle machine by using a cycle graph. In fact, we'll begin our study of activation cycle machines by proving just that.\\
\\
Our definition then extends beyond cycle graphs by incorporating the idea of \textit{activators}, which simply are the first $a$ states of the machine, $s_0, ..., s_{a-1}$. Our definition also stipulates that while there is always at least one activator, there are never as many activators as there are states in the machine. As such, every activation cycle machine will become \textit{active} at least once and \textit{inactive} at least once as it transitions between its states.\\
\\
Before we dive into our study of activation cycle machines, below you will find two working examples of an activation cycle machine coded in \textit{ruby}. The first example is easier to follow, it relies on the fact that we can keep track of our \textit{position} using arithmetic, rather than keeping track of a bunch of states. Our second example, on the other hand, is represented using states, and as such, needn't rely on arithmetic to function.
\begin{tcolorbox}
\begin{verbatim}
...
\noindent
\\
As you can see, our first example simply increments the position of our state machine by one to simulate a transition, with the exception that it resets the position to 0 once its gotten to the maximum position.\\
\\
We'll need to break our second example into two seperate classes. The first class we will define will represent a single state in our machine, and will be implemented as a node in a linked list would. Our second class will be the actual activation cycle machine, which we will see is implemented much the same way that a linked list would be.\\
\\
\begin{tcolorbox}
\begin{verbatim}
class State
