...
\\
Furthermore, we define the \textit{index walk function}, $\Omega: (P, \mathbb{N}) \rightarrow \mathbb{N}$ to be $\Omega(p_i, k) = j$, where $j$ is the index of the point $\omega(p_i, k)$.
\end{tcolorbox}
\noindent
\\
...
\\
Now let us wrap our our uniqueness by assuming $\Omega(p_i, 1) = \Omega(p_j, 1) = k$ for some $k > 0$, and showing that this results in $p_i = p_j$. By the definition of a cycle graph, we know that point $p_{k-1}$ has only a single edge which goes to point $p_k$, thus $\omega(p_{k-1}, 1) = p_k$. Furthermore, for any $l < k - 1$, $\Omega(p_l, 1) = l + 1 \leq k - 1 < k$. Thus, it must be the case that $p_{k-1}$ is the only point that allows us to take a walk of length one to arrive at point $p_k$.\\
\\
\textit{Hypothesis}\\
