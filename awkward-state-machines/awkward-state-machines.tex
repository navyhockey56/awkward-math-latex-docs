...
\begin{itemize}
\item $\psi(G_n, 0, k) = \omega(p_0, k)$ for any $k \in N$.
\item $\psi(G_n, j, k) = \psi(G_n, j - 1, k + 1)$ for any $j \in \mathbb{N^+}$ and any $k \in \mathbb{N}$.
\end{itemize}
...
\noindent
\\
The state position function is much simpler than it might first appear. Whenever the step is $0$, $\psi$ delegates to the walk function which we already know quite a bit about. Whenever the step is a positive number, we will be able to repeatedly apply the rule $\psi(G_n, j, k) = \psi(G_n, j - 1, k + 1)$ until we've reduced the step to $0$, once again leaving us with the walk function to deal with. Within our study, we will find that the step argument is simply acting as a memory.\\
...
\begin{tcolorbox}
\textbf{Definition}\\
For any cycle graph $G_n$, for any $j, k \in \mathbb{N}$, we define the \textit{state function}
\begin{center}
$\Psi : (G_n, \mathbb{N}, \mathbb{N}) \rightarrow [n]$
\end{center}
as
\begin{center}
$\Psi(G_n, j, k) = \Omega(\psi(G_n, j, k))$ 
\end{center}

\noindent If $\Psi(G_n, j, k) = t$, we say $G_n$ is on \textit{state} $t$ after $k$ transitions from step $j$.
