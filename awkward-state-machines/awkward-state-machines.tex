...
\noindent
\\
In order to do a proper study of awkward state machines, we will need to borrow some well established theory from outside of this paper. We will not bother proving this theory ourselves, and instead, accept it as fact. However, we will explicitly state when we are doing so.
\begin{tcolorbox}
\textbf{Outside Theory - Theorem \& Definition}\\
For any positive integer $n$, for any integer $t \geq 0$, there exists a unique integer $q \geq 0$, and a unique integer $r \in [n]$ such that $t = qn + r$.\\
\\
...
\\
If $\rho(n, t) + i < n$, then $\Omega(p_i, \rho(n, t)) = i + \rho(n, t) = \rho(n, i + \rho(n, t)) = \rho(n, i + t)$\\
\\
If $\rho(n, t) + i \geq n$, then $\Omega(p_i, \rho(n, t)) = \rho(n, t) - (n - i) = \rho(n, t) + i - n = (n + k) - n = k$ where $k = n - (\rho(n, t) + i)$\\
\\
$\rho(n, t) = t - qn$ and $\rho(n, i + t) = (i + t) - kn$\\
Sub $\rho(n, i + t) = (i + \rho(n, t) + qn) - kn = i + \rho(n, t) + (q - k)n$.\\
